...
{
	if (pFirst == nullptr) throw "Empty list"; // throw (done)
	...
	TNode<TKey, TData>* tmp = pFirst;
	while (tmp->pNext != nullptr && tmp->pNext->key != searchKey) // && (done)
		tmp = tmp->pNext;
...
void TList<TKey, TData>::InsertAfter(TKey searchKey, TKey newKey, TData* newData)
{
	if (pFirst == nullptr) throw "Empty list"; // throw (done)
	TNode<TKey, TData>* tmp = pFirst;
	while (tmp != nullptr && tmp->key != searchKey) // && (done)
		tmp = tmp->pNext;
	if (tmp == nullptr) throw "There is no element with such a key";
	TNode<TKey, TData>* newNode = new TNode<TKey, TData>(newKey, newData);
	newNode->pNext = tmp->pNext;
...
template <class TKey, class TData>
void TList<TKey, TData>::Remove(TKey searchKey) // simplify (done)
{
	if (pFirst == nullptr) throw "Empty list"; // throw (done)
	if (pFirst->key == searchKey)
	{
...
{
	return (pCurrent == nullptr); // pCurrent (done)
}
...
void TList<TKey, TData>::MoveNext()
{
	...
	pCurrent = pNext;
	if (pNext != nullptr)
		pNext = pNext->pNext; // pNext == nullptr (done)
	return;
}
